document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchCollegeDetails(collegeName) {
        ...
        for (let i = 0; i < 3; i++) {
            let emailPromptMessage = "Please enter your email first before getting your quote";
            let email = window.sessionStorage.getItem("session-email");

            if (!email) {
                const tempEmail = window.prompt(emailPromptMessage);

                if (!tempEmail && tempEmail === "") {
                    window.alert("No email inputted.");
            ...
            validateEmail(email).then(is_email_valid => {
                if (is_email_valid) {
                    ...
                    window.alert("Inputted Email must be a deliverable valid email, or the data won't show.");

                    emailPromptMessage = "Please input a valid deliverable email. Attempt " + (i + 1) + " of 3."; 
                }
            }).catch(err => {
