document.addEventListener("DOMContentLoaded", function () {
  ...
  var timeToRecoupInStateField = document.getElementById(
    "timeToRecoupFundsInState",
  );
  var timeToRecoupOutOfStateField = document.getElementById(
    "timeToRecoupFundsOutOfState",
  );
  var leftoverSavings2 = document.getElementById("leftoverSavings");
  var inStateTuitionTwoYearsField = document.getElementById(
    "inStateTuitionTwoYears",
  );
  var outOfStateTuitionTwoYearsField = document.getElementById(
    "outOfStateTuitionTwoYears",
  );
  var potentialSavingsInStateField = document.getElementById(
    "potentialSavingsInState",
  );
  var potentialSavingsOutOfStateField = document.getElementById(
  ...
  var emailPopUpFormErrorMessage = document.getElementById(
    "email-error-message",
  );
  var emailPopUpFormSubmitButton = document.getElementById("email-form-submit");
  var phoneNumberField = this.getElementById("phone-form");

  let selectedCollegeName = undefined;
  // This flag will only be true when the user have just validated their email
  let emailShouldBeInserted = false;
  ...
  function toggleCells(selectedStateStatus) {
    ...
    var cellsToHide =
      selectedStateStatus === "inState" ? ["1", "2", "6"] : ["3", "4", "5"];
    var cellsToShow =
      selectedStateStatus === "inState" ? ["3", "4", "5"] : ["1", "2", "6"];
  ...
  emailPopUpForm.addEventListener("submit", (ev) => {
    ...
    emailShouldBeInserted = true;

    closeModal();
    fetchCollegeDetails(selectedCollegeName);
  });

  async function insertEmailAndChoice() {
    const endpointUrl =
      "https://us-east-1.aws.data.mongodb-api.com/app/roitoolapp-jnjed/endpoint/insertEmailDocument";

    const email = window.sessionStorage.getItem("session-email");

    // If there's no email, no need to insert it to the database
    if (!email || !emailShouldBeInserted) return;
    ...
    await fetch(endpointUrl, {
      ...
      .then((reply) => {
        console.log("Successfully inserted user choice!", reply);
        emailShouldBeInserted = false;
      })
      .catch((err) => {
  ...
  function fetchCollegeDetails(collegeName) {
    var endpointUrl =
      "https://us-east-1.aws.data.mongodb-api.com/app/roitoolapp-jnjed/endpoint/collegeDetails?college=" +
    ...
    fetch(endpointUrl)
      ...
      .then((data) => {
        ...
        if (
          ...
          inStateTuitionField.textContent =
            "$" + (data.latest_cost_tuition_in_state * 4).toLocaleString();
          outOfStateTuitionField.textContent =
            "$" + (data.latest_cost_tuition_out_of_state * 4).toLocaleString();

          // Update the salary fields
          averageSalary6Field.textContent =
            "$" +
            data.latest_earnings_6_yrs_after_entry_median.toLocaleString();
          averageSalary10Field.textContent =
            "$" +
            data.latest_earnings_10_yrs_after_entry_median.toLocaleString();

          fetchCostOfLivingData(stateSelect.value);
        } else {
          console.error(
            "Some necessary data is missing for the selected college",
          );
        }
      })
      .catch((error) => {
  ...
  function createDefaultOptions() {
    ...
    option.value = "Select a college..."; // Use the college name as the value
  ...
  stateSelect.addEventListener("change", function () {
    ...
    collegeSelect.appendChild(createDefaultOptions());

    var endpointUrl =
      "https://us-east-1.aws.data.mongodb-api.com/app/roitoolapp-jnjed/endpoint/colleges?state=" +
      encodeURIComponent(stateAbbreviation);
  ...
  function fetchCostOfLivingData(state) {
    var endpointUrl =
      "https://us-east-1.aws.data.mongodb-api.com/app/roitoolapp-jnjed/endpoint/costOfLiving?state=" +
    ...
    fetch(endpointUrl)
      ...
      .then((data) => {
        ...
        const averageSalary6 = parseInt(
          averageSalary6Field.textContent.replace(/\$/g, "").replace(/,/g, ""),
          10,
        );
        const averageSalary10 = parseInt(
          averageSalary10Field.textContent.replace(/\$/g, "").replace(/,/g, ""),
          10,
        );
        ...
        if (leftoverSavings < 0) {
          timeToRecoupInStateField.textContent =
            "No leftover savings per year, payoff will be extremely difficult";
          timeToRecoupOutOfStateField.textContent =
            "No leftover savings per year, payoff will be extremely difficult";
        } else {
          // Calculate time to recoup funds for in-state tuition
          const inStateTuition = parseInt(
            inStateTuitionField.textContent
              .replace(/\$/g, "")
              .replace(/,/g, ""),
            10,
          );
          const yearsToRecoupInState = Number(
            Math.floor(inStateTuition / leftoverSavings),
          ).toLocaleString("en", { useGrouping: true });
          timeToRecoupInStateField.textContent =
            yearsToRecoupInState + " years";

          // Calculate time to recoup funds for out-of-state tuition
          const outOfStateTuition = parseInt(
            outOfStateTuitionField.textContent
              .replace(/\$/g, "")
              .replace(/,/g, ""),
            10,
          );
          const yearsToRecoupOutOfState = Number(
            Math.floor(outOfStateTuition / leftoverSavings),
          ).toLocaleString("en", { useGrouping: true });
          timeToRecoupOutOfStateField.textContent =
            yearsToRecoupOutOfState + " years";
        ...
        const inStateTuition = parseInt(
          inStateTuitionField.textContent.replace(/\$/g, "").replace(/,/g, ""),
          10,
        );
        const outOfStateTuition = parseInt(
          outOfStateTuitionField.textContent
        ...
        potentialSavingsInStateField.textContent =
          "$" +
          (inStateTuition / 2).toLocaleString("en", { useGrouping: true });
        potentialSavingsOutOfStateField.textContent =
          "$" +
          (outOfStateTuition / 2).toLocaleString("en", { useGrouping: true });
